...
const slugifyPageName = function (value) {
  return value
    .toLowerCase()
    .trim()
    .normalize("NFD")
...
const setActivePage = function (targetPageName) {
  for (let i = 0; i < pages.length; i++) {
    const pageName = pages[i].dataset.page;
    const isActive = pageName === targetPageName;
...
const findPageNameFromHash = function () {
  const hash = window.location.hash.replace("#", "").trim();
  if (!hash) return null;

  for (let i = 0; i < pages.length; i++) {
    const pageName = pages[i].dataset.page;
    if (slugifyPageName(pageName) === hash) {
      return pageName;
    }
  }
...
for (let i = 0; i < navigationLinks.length; i++) {
  navigationLinks[i].addEventListener("click", function (event) {
    ...
    const targetPageName = this.textContent.toLowerCase().trim();
...
window.addEventListener("DOMContentLoaded", function () {
  const pageFromHash = findPageNameFromHash();
  if (pageFromHash) {
