  ...
  form.addEventListener('submit', async function(e) {
    ...
    try {
      ...
      if (_0xm2n3o4 > 0 && _0x7g8h9i()) {
        ...
        // Données du formulaire avec validation
        const formData = new FormData(form);
        const _0xp5q6r7 = {
          name: formData.get('fullname')?.substring(0, 100) || '',
          email: formData.get('email')?.substring(0, 100) || '',
          message: formData.get('message')?.substring(0, 1000) || '',
          timestamp: new Date().toISOString(),
          source: 'Portfolio Contact Form'
        };
        
        // Validation côté client
        if (!_0xp5q6r7.name.trim() || !_0xp5q6r7.email.includes('@') || !_0xp5q6r7.message.trim()) {
        ...
        if (WEBHOOK_URL && WEBHOOK_URL !== 'YOUR_WEBHOOK_URL_HERE') {
          try {
            await sendWebhook(_0xp5q6r7);
            discordSuccess = true;
            console.log('✅ Message envoyé sur Discord avec succès');
          } catch (discordError) {
            console.warn('⚠️ Webhook Discord échoué:', discordError.message);
        ...
        if (discordSuccess || formspreeSuccess) {
          ...
          throw new Error('Aucun service de messagerie n\'a répondu');
        }
      } else {
